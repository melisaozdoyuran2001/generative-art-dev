...
(function () {
  let bot = {
    ...
    hide: false,
    name: "talkingbot", // Lowercase only no spaces! (we reuse this for some Vue stuff)
    description: "a bot that talks",
    ...
    setup() {
      ...
      console.log(this.getEmojiByText("square is cool"))

      //-------------------------------
      // SPEECH
      // Enable your browser's built in speech
      console.log("Set up speech synthesis");
      this.speechSynthesis = window.speechSynthesis;

      // This bot will post, but won't speak until the user has interacted once with the page
      this.botSay("Hello, I am a talking bot");
      this.botSay("I can talk once you say something");

      //--------------------------
      // SPEECH RECOGNITION
      console.log("Set up speech recognition");
    ...
    startListening() {
      ...
      this.recognition.onresult = (event) => {
        ...
        this.heardText = transcript;
        console.log(this.heardText);
    ...
    stopListening() {
      ...
      // Use this as our input
       this.input({ text: this.heardText, from: "user" });
        this.heardText = "";
    ...
    getEmojiByText(text) {
      /**
      * get an emoji that matches this text
      **/
      
      text = text.toLowerCase().replace(/\W/g, ' ')
      let words = text.split(" ")
      console.log(text)
      let matching = ALL_EMOJI.filter(emoji => {
        if (emoji.description.includes(text)) 
          return true
        if (emoji.aliases.some(t => words.includes(t))) {
          return true
        }
         if (emoji.tags.some(t => words.includes(t))) {
          return true
        }
      })
      return matching
    ...
    botSay(text) {
      // A helper function, because this bot bot adds to message, and also talks
    ...
    input({ text, from, otherDataHere }) {
      ...
      // For this bot, get matching emoji
      let matchingEmoji = []
      let textToSay = "Can you say an emoji?"
      if (text.length > 0) {
        matchingEmoji = this.getEmojiByText(text)
        if (matchingEmoji)
          textToSay = matchingEmoji.map(e => e.emoji).join("")
  ...
  Vue.component(`panel-${bot.name}`, {
    template: `<div>
			...
			<h1 v-if="bot.isListening">LISTENING</h1>
      <h1 v-else>NOT LISTENING</h1>
  ...
  Vue.component(`input-${bot.name}`, {
    ...
    template: `<div>
      ...
      <button @mousedown="bot.startListening()"  @mouseup="bot.stopListening()">🎤</button>
